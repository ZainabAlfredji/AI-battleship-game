...
public class SelectionGrid extends Rectangle {
     ...
     */
    public SelectionGrid(int x, int y, BattleshipManager battleshipManager) {
        super(x, y, CELL_SIZE*GRID_WIDTH, CELL_SIZE*GRID_HEIGHT);
        this.battleshipManager = battleshipManager;
        createMarkerGrid();
        ships = new ArrayList<>();
        battleShips = battleshipManager.getBattleShips();
        rand = new Random();
        showShips = false;
    }
    ...
    public void paint(Graphics g) {
        for(Ship ship : ships) {
            if(showShips || GameController.debugModeActive || ship.isDestroyed()) {
                ship.paint(g);
            }
        }
        drawMarkers(g);
    ...
    /**
     * Resets the Model.SelectionGrid by telling all the markers to reset,
     ...
     * all been destroyed.
     *
     * @param posToMark Model.Position to mark.
     ...
     *
     * @param posToTest Model.Position to test if it is marked.
     ...
     *
     * @param posToSelect Model.Position on the grid to select the marker at.
    ...
    public void populateShips() {
        ...
        for(int i = 0; i < battleShips.size(); i++) {
            ...
            int gridX,gridY;
            do {
                gridX = rand.nextInt(sideways?GRID_WIDTH-battleShips.get(i).getSize():GRID_WIDTH);
                gridY = rand.nextInt(sideways?GRID_HEIGHT:GRID_HEIGHT-battleShips.get(i).getSize());
            } while(!canPlaceShipAt(gridX,gridY,battleShips.get(i).getSize(),sideways));
            placeShip(gridX, gridY, battleShips.get(i).getSize(), sideways);
        }
    }
    ...
    /**
     * @return - totalMarks is number of times a player has selected a grid
     */
