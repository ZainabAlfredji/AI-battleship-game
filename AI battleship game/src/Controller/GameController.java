...
public class GameController extends JPanel implements MouseListener, MouseMotionListener {
     ...
     */
    public GameController(int aiChoice, Player currentPlayer) {
        this.currentPlayer = currentPlayer;
        battleshipManager = new BattleshipManager();
        highScore = new HighScore();
        computer = new SelectionGrid(0,0, battleshipManager);
        player = new SelectionGrid(0,computer.getHeight()+50, battleshipManager);
    ...
    public void restart() {
        ...
        //Creates a new player object with the same name as before
        ...
        placingShip = new Ship(new Position(0,0),
                ...
                battleshipManager.getBattleShips().get(0).getSize(), true);
        placingShipIndex = 0;
        updateShipPlacement(tempPlacingPosition);
    ...
    private void tryPlaceShip(Position mousePosition) {
        ...
        if(player.canPlaceShipAt(targetPosition.x, targetPosition.y,
                battleshipManager.getBattleShips().get(placingShipIndex).getSize(),placingShip.isSideways())) {
    ...
    private void placeShip(Position targetPosition) {
        ...
        if(placingShipIndex < battleshipManager.getBattleShips().size()) {
            placingShip = new Ship(new Position(targetPosition.x, targetPosition.y),
                    ...
                    battleshipManager.getBattleShips().get(placingShipIndex).getSize(), true);
            updateShipPlacement(tempPlacingPosition);
        } else {
    ...
    private void doPlayerTurn(Position targetPosition) {
        ...
        if(hit && computer.getMarkerAtPosition(targetPosition).getAssociatedShip().isDestroyed()) {
            Ship tempShip = computer.getMarkerAtPosition(targetPosition).getAssociatedShip();
            destroyedType = tempShip.toString();
            destroyed = "(Destroyed)" + destroyedType;
        }
        ...
        if(computer.areAllShipsDestroyed()) {
            ...
            currentPlayer.setScore(player.getTotalMarks());
            highScore.addHighScore(currentPlayer);
            highScoreDialog = new HighScoreDialog(new HighScore());
        }
    }
    ...
    private void doAITurn() {
        ...
        if(hit && player.getMarkerAtPosition(aiMove).getAssociatedShip().isDestroyed()) {
            Ship tempShip = player.getMarkerAtPosition(aiMove).getAssociatedShip();
            destroyedType = tempShip.toString();
            destroyed = "(Destroyed)" + destroyedType;
        }
    ...
    private void updateShipPlacement(Position targetPos) {
        ...
        if(placingShip.isSideways()) {
            targetPos.x = Math.min(targetPos.x, SelectionGrid.GRID_WIDTH - battleshipManager.getBattleShips().get(placingShipIndex).getSize());
        } else {
            targetPos.y = Math.min(targetPos.y, SelectionGrid.GRID_HEIGHT - battleshipManager.getBattleShips().get(placingShipIndex).getSize());
        }
